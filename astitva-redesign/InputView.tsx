...
export default function InputView({ onSubmit, onTestMode, error }: InputViewProps) {
  ...
  const handleSubmit = () => {
    const raw = url.trim()
    if (!raw) return
    onSubmit(raw.startsWith('http') ? raw : `https://${raw}`)
  }
      ...
      <div className="relative z-10 w-full max-w-lg px-6">
        ...
        >
          <div
            ...
            style={{
              background: focused
                ? 'linear-gradient(135deg, rgba(124,58,237,0.6), rgba(37,99,235,0.5))'
                : 'rgba(255,255,255,0.07)',
            }}
          >
            <div
              className="rounded-[15px] p-5 space-y-3"
            ...
            >
              ...
              <div className="flex gap-2">
                <button
                  ...
                  disabled={!url.trim()}
                  className="flex-1 py-2.5 rounded-xl text-sm font-semibold transition-all duration-200 cursor-pointer disabled:opacity-40 disabled:cursor-not-allowed"
                  style={{
                ...
                <button
                  onClick={onTestMode}
                  className="px-4 py-2.5 rounded-xl text-sm font-medium transition-all duration-200 cursor-pointer flex items-center gap-1.5 whitespace-nowrap hover:opacity-80"
                  style={{
              ...
              <p className="text-xs text-center" style={{ color: 'var(--color-text-tertiary)' }}>
                Demo uses pre-loaded data for{' '}
                <span style={{ color: '#a78bfa' }}>devalok.in</span>
                {' '}— no API key needed
              </p>
            </div>
          </div>
